...
void Demo::createTextureImage()
{
	...
	//将会在生成mip贴图的过程中将image布局转为着色器只读布局
	/*transitionImageLayout(textureImage,
						  VK_FORMAT_R8G8B8A8_UNORM,
...
void Demo::createImage(uint32_t width, uint32_t height, uint32_t mipLevels,
					   VkFormat format,
					   ...
					   VkMemoryPropertyFlags properties, VkImage & image, VkDeviceMemory & memory)
{
	...
	imageInfo.mipLevels = mipLevels; //mip贴图层次
	...
	imageInfo.samples = VK_SAMPLE_COUNT_1_BIT; //用于多重采样的数量，此处只有一个image
...
VkImageView Demo::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels)
{
...
void Demo::generateMipmaps(VkImage image, VkFormat imageFormat, int texWidth, int texHeight, uint32_t mipLevels)
{
	//检查image的format是否支持linear blit操作
...
//转换image布局
void Demo::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels)
{
	VkCommandBuffer commandBuffer = beginTempCommands();
